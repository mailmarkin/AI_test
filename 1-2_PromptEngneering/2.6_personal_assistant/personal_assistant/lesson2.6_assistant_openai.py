...
from src import OPENAI_API_KEY, ASSISTANT_ID, \
    TELEGRAM_TOKEN  # Импортируем необходимые ключи и токены из локального модуля src
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Привет! Я бот-помощник на OpenAI.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "Ошибка при обработке запроса. Попробуйте позже.")  # Отправляем сообщение об ошибке пользователю
